#!/usr/bin/env tsx
/**
 * Croatian Law MCP -- Ingestion Pipeline
 *
 * Fetches Croatian legislation from the Sejm ELI API (api.sejm.gov.pl).
 * The Sejm (Croatian Parliament) provides free public access to all legislation
 ...
 * License: Croatian legislation is public domain under Art. 4 of the Copyright Act
 */
...
import { parseCroatianHtml, KEY_CROATIAN_ACTS, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function fetchAndParseActs(acts: ActIndexEntry[], skipFetch: boolean): Promise<void> {
  console.log(`\nProcessing ${acts.length} Croatian Acts from api.sejm.gov.pl...\n`);
  ...
  for (const act of acts) {
    ...
    try {
      ...
      const parsed = parseCroatianHtml(html, act);
  ...
  console.log(`  License:     Public domain (Art. 4 Croatian Copyright Act)`);
...
async function main(): Promise<void> {
  ...
  const acts = limit ? KEY_CROATIAN_ACTS.slice(0, limit) : KEY_CROATIAN_ACTS;
