#!/usr/bin/env tsx
/**
 * Croatian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://narodne-novine.nn.hr';
const PORTAL_NAME = 'Narodne novine';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/croatian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Croatian Law MCP — Data Freshness Check');
