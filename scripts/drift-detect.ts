#!/usr/bin/env tsx
/**
 * Drift detection for Croatian Law MCP.
 *
 * Checks if upstream www.nn.hr content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Croatian Law MCP — Drift Detection');
  console.log('=====================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Croatian-Law-MCP/1.0 drift-detect' },
      });
