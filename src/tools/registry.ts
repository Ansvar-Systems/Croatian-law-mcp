/**
 * Tool registry for Croatian Law MCP Server.
...
import { getCroatianImplementations, type GetCroatianImplementationsInput } from './get-croatian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Narodne novine (Official Gazette) (Croatian Government, Office of Parliamentary Counsel). ' +
    ...
    'Call this FIRST when you need to understand what Croatian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Croatian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Croatia?" ' +
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Croatian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_croatian_implementations',
    description:
      'Find all Croatian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Croatian statutes. ' +
      'Note: Croatia is an EU Member State and transposes EU directives into national law.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Croatian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_croatian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Croatian legislation.',
        },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_croatian_implementations':
          result = await getCroatianImplementations(db, args as unknown as GetCroatianImplementationsInput);
          break;
