/**
 * Tests for Croatian Law MCP tools.
...
describe('database integrity', () => {
  it('should have at least 4,500 legal documents (full corpus)', () => {
    const row = db.prepare('SELECT COUNT(*) as cnt FROM legal_documents').get() as { cnt: number };
    expect(row.cnt).toBeGreaterThanOrEqual(4500);
  });
...
describe('GDPR Implementation Act', () => {
  it('should find GDPR Implementation Act', () => {
    const row = db.prepare(
      "SELECT id FROM legal_documents WHERE id = 'gdpr-impl-nn-42-18'"
    ).get() as { id: string } | undefined;
    expect(row).toBeDefined();
  });

  it('should have art1 mentioning Regulation (EU) 2016/679', () => {
    const row = db.prepare(
  ...
  it('should have GDPR EU reference', () => {
    const row = db.prepare(
      "SELECT eu_document_id FROM eu_references WHERE document_id = 'gdpr-impl-nn-42-18' AND eu_document_id LIKE '%2016/679%'"
...
describe('Cybersecurity Act', () => {
  it('should find Cybersecurity Act', () => {
    const row = db.prepare(
      "SELECT id FROM legal_documents WHERE id = 'cybersecurity-nn-14-24'"
    ).get() as { id: string } | undefined;
    expect(row).toBeDefined();
  });
...
describe('negative cases', () => {
  ...
  it('should return no results for non-existent provision', () => {
    const row = db.prepare(
      "SELECT content FROM legal_provisions WHERE document_id = 'gdpr-impl-nn-42-18' AND provision_ref = '999ZZZ'"
    ).get();
    expect(row).toBeUndefined();
  });
});
